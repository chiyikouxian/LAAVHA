...
\Author{Hanming Sun $^{1}$, Tong Liu $^{2,}$*, Wen Su $^{2}$, Jie Lin $^{2}$, Junsong Luo $^{2}$ and Bin Duo $^{2}$}

\AuthorNames{Hanming Sun, Tong Liu, Wen Su, Jie Lin, Junsong Luo and Bin Duo}

% 孙汉明单位：济宁学院机电工程学院；其余五位作者单位：成都理工大学计算机与网络安全学院。
\address{%
$^{1}$ \quad School of Mechanical and Electrical Engineering, Jining University, Jining City, Shandong Province, 273155, China; 202210043@jnxy.edu.cn (H.M.S.)\\
$^{2}$ \quad School of Computer and Network Security, Chengdu University of Technology, Chengdu 610059, China;\\
\phantom{$^{2}$ \quad }liutong1982@cdut.edu.cn (T.L.); 2168468713@qq.com (W.S.); linjie07@cdut.edu.cn (J.L.);\\
\phantom{$^{2}$ \quad }luojuns@cdut.edu.cn (J.S.L.); duobin@cdut.edu.cn (B.D.)}

% 通讯作者：刘同 Tong Liu。电话为可选项，按模板要求留空。
\corres{Correspondence: liutong1982@cdut.edu.cn}
